...
def sample_n_unique(sampling_f, n):
    ...
    """
    # revised by ZY using OrderedDict for faster check
...
class PiecewiseSchedule:
    def __init__(self, endpoints, interpolation=linear_interpolation,
        ...
        endpoints: [(int, int)]
            ...
            an increasing order. When t is between two times, e.g. `(time_a,
            value_a)` and `(time_b, value_b)`, such that `time_a <= t < time_b`
            then value outputs `interpolation(value_a, value_b, alpha)` where
            alpha is a fraction of time passed between `time_a` and `time_b`
            for time `t`.
        ...
        outside_value: float
            If the value is requested outside of all intervals sepecified in
...
def initialize_interdependent_variables(session, vars_list, feed_dict):
    ...
    while len(vars_left) > 0:
        ...
        if len(new_vars_left) >= len(vars_left):
            # This can happend if the variables all depend on each other, or
            # more likely if there's another variable outside of the list, that
            # still needs to be initialized. This could be detected here, but
            # life's finite.
            raise Exception("Cycle in variable dependencies, or extenrnal \
...
class ReplayBuffer:
    def __init__(self, size, frame_history_len):
        ...
        For the typical use case in Atari RL buffer with 1M frames the total
    ...
    def _encode_sample(self, idxes):
        obs_batch = np.concatenate([self._encode_observation(idx)[None]
                                   for idx in idxes], 0)
        act_batch = self.action[idxes]
        rew_batch = self.reward[idxes]
        next_obs_batch = np.concatenate([self._encode_observation(idx+1)[None]
                                        for idx in idxes], 0)
    ...
    def _encode_observation(self, idx):
        ...
        if start_idx < 0 or missing_context > 0:
            ...
            # this optimization has potential to save about 30% compute time
    ...
    def store_effect(self, idx, action, reward, done):
        ...
        at index idx. The reason `store_frame` and `store_effect` is broken up
        into two functions is so that once can call `encode_recent_observation`
        in between.
